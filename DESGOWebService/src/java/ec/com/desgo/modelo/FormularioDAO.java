...
public class FormularioDAO implements CRUDFormulario{
    ...
    public boolean registrarFormulario(User us, FormularioIds form){
        ...
        try {
            PreparedStatement ps = accesoDB.prepareStatement("insert into `formulario`(`ID_FORMULARIO`,`ID_IULOTE`,`ID_USUARIO`,`CODIGO_FORMULARIO`,`ESTADO_FORMULARIO`) values (null,?,?,?,?)");
            ps.setInt(1, form.getIdentificacionU_F());
            ps.setInt(2, form.getIdUsuario_F());
            ps.setInt(3, form.getCodigo_F());
            ps.setInt(4, form.getEstado_F());
